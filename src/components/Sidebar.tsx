...
export default function Sidebar() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--bg-surface)', borderRight: '1px solid var(--border)' }}>
      {/* Header / Logo */}
      <div style={{ padding: '18px 16px 12px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>Desktop Agent</div>
          </div>
        </div>
      </div>

      {/* Status Card */}
      <div style={{ margin: '12px 10px', padding: '10px 12px', background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 10 }}>
        ...
        <button
          ...
          style={{ width: '100%', background: 'var(--bg-hover)', border: '1px solid var(--border)', borderRadius: 6, padding: '5px 9px', textAlign: 'left', cursor: 'pointer', color: 'var(--text-secondary)', fontSize: 11.5, fontFamily: 'monospace', display: 'flex', alignItems: 'center', justifyContent: 'space-between', transition: 'border-color 0.15s', overflow: 'hidden' }}
          onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = 'var(--accent)'; }}
          onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = 'var(--border)'; }}
        >
          <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 }}>{activeModel}</span>
          <Cpu size={11} style={{ flexShrink: 0, marginLeft: 4, color: 'var(--accent)' }} />
        </button>
        {tokensUsed > 0 && contextWindow > 0 && (
          <div style={{ marginTop: 8 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
              <span style={{ fontSize: 10, color: 'var(--text-muted)' }}>Context</span>
              <span style={{ fontSize: 10, color: 'var(--text-muted)' }}>{Math.round((tokensUsed / contextWindow) * 100)}%</span>
            </div>
            <div style={{ height: 3, background: 'var(--bg-base)', borderRadius: 2 }}>
              <div style={{ height: '100%', width: `${Math.min((tokensUsed / contextWindow) * 100, 100)}%`, background: 'var(--accent)', borderRadius: 2, transition: 'width 0.4s' }} />
            </div>
          </div>
        )}
      </div>
      ...
      <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        <button
          ...
          style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 16px', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', fontSize: 11, letterSpacing: '0.06em', textTransform: 'uppercase', fontWeight: 600 }}
        >
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '0 8px' }}>
            {sessions.map((s) => (
              <button
                ...
                onMouseEnter={e => { if (s.id !== activeSessionId) (e.currentTarget as HTMLElement).style.background = 'var(--bg-hover)'; }}
              ...
              >
                ...
                <div style={{ fontSize: 10.5, color: 'var(--text-muted)', marginTop: 1 }}>{formatTimestamp(s.timestamp)}</div>
              </button>
            ))}
          </div>
        )}
